...
//            System.out.println(area + " : " + h);
    ...
    public double[] getMinDistanceFromPlace(JdbcRowSet rs,String area, int easting, int northing,int radius)throws Exception
    {
    ...
    public void createTrainingSet(Statement stmt,Connection cn,JdbcRowSet parks,JdbcRowSet monuments,JdbcRowSet buildings)throws Exception
    {
              ...
              //  System.out.println("Item: " + counter + " / " + yd.getData().size() + " (" + Math.round((float)counter / yd.getData().size()) + "%)");
                ...
                if (RS.getString("NGRCode").equalsIgnoreCase(area))
                {
                   // double[] latlon = Common.enToLatLon(String.valueOf(s.getE()), String.valueOf(s.getN()));
                    ...
                    double height = getHeight(area, RS.getInt("Easting"), RS.getInt("Northing"));
                    ...
                    double dist_n_CountM[] = getMinDistanceFromPlace(monuments, area,Math.round(RS.getFloat("Easting")), Math.round(RS.getFloat("Northing")),5000);
                    double dist_n_CountP[] = getMinDistanceFromPlace(parks, area, Math.round(RS.getFloat("Easting")), Math.round(RS.getFloat("Northing")),5000);
                    double dist_n_CountB[] = getMinDistanceFromPlace(buildings, area,Math.round(RS.getFloat("Easting")), Math.round(RS.getFloat("Northing")),5000);
                    
                    
                  //  double countM = countObjectsWithinRadius(DataSetType.MONUMENTS, area, Math.round(RS.getFloat("Easting")), Math.round(RS.getFloat("Northing")), 5000);
                  //  double countP = countObjectsWithinRadius(DataSetType.PARKS, area, Math.round(RS.getFloat("Easting")), Math.round(RS.getFloat("Northing")), 5000);
                  //  double countB = countObjectsWithinRadius(DataSetType.BUILDINGS, area, Math.round(RS.getFloat("Easting")), Math.round(RS.getFloat("Northing")), 5000);
               // System.out.println(RS.getString("NGRCode"));
                      writer.write(0 + 
                            "," + height + 
                            "," + dist_n_CountM[0] + 
                            "," + dist_n_CountP[0] + 
...
//                System.out.println(eastingMod + " " + northingMod + " " + easting + " " + northing);
    ...
    public void createTestSet(JdbcRowSet parks,JdbcRowSet monuments,JdbcRowSet buildings)
    {
        BufferedWriter writer = null;
...
//                        double[] latlon = Common.enToLatLon(String.valueOf(p.getEasting()), String.valueOf(p.getNorthing()));
                     ...
                     // two values are returned , 
                        ...
                        double distM_n_Count[] = getMinDistanceFromPlace(monuments, area, p.getEasting(), p.getNorthing(),5000);
                        double distP_n_Count[] = getMinDistanceFromPlace(parks, area, p.getEasting(), p.getNorthing(),5000);
                        double distB_n_Count[] = getMinDistanceFromPlace(buildings, area, p.getEasting(), p.getNorthing(),5000);
